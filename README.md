...
final class DisclaimerCell: UICollectionViewCell {
    ...
    func configure(with model: DisclaimerModel) {
        ...
        // Especificar tanto font como textStyle para asegurar escalabilidad
        textView.setAttributedTextAsync(
            text: model.subtitle,
...
internal extension String {
    ...
    func toAttributedStringAsync(
        font: UIFont? = nil,
        textStyle: UIFont.TextStyle? = nil,
        color: UIColor
    ) async throws -> NSAttributedString {
        
        // Prioridad: textStyle > font con textStyle > font sin textStyle
        let finalFont: UIFont
        
        if let textStyle = textStyle, let baseFont = font {
            finalFont = UIFontMetrics(forTextStyle: textStyle).scaledFont(for: baseFont)
        } else if let font = font {
            // Si solo tenemos font, intentar detectar si tiene un textStyle asociado
            if let descriptor = font.fontDescriptor.fontAttributes[.textStyle] as? UIFont.TextStyle {
                // El font tiene un textStyle asociado, usarlo para escalar
                finalFont = UIFontMetrics(forTextStyle: descriptor).scaledFont(for: font)
            } else {
                // Font sin textStyle, usar tal cual (no escalará dinámicamente)
                finalFont = font
            }
        } else {
            // Sin font ni textStyle, usar body por defecto
            finalFont = UIFont.preferredFont(forTextStyle: .body)
        }
        
        let htmlText = """
            <style>
                body {
                    font-family: '\(finalFont.fontName)';
                    font-size: \(finalFont.pointSize)px;
        ...
        guard let data = htmlText.data(using: .utf8) else {
            throw AttributedStringError.dataEncodingFailed
        }

        // Usar withCheckedThrowingContinuation para manejar el trabajo en background
        return try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                do {
                    ...
                    // Crear una versión mutable para aplicar los atributos de fuente escalable
                    let mutableAttributedString = NSMutableAttributedString(attributedString: attributedString)
                    let fullRange = NSRange(location: 0, length: mutableAttributedString.length)
                    
                    // Aplicar la fuente escalable a todo el texto
                    mutableAttributedString.addAttribute(.font, value: finalFont, range: fullRange)
                    
                    // Preservar el color original
                    mutableAttributedString.addAttribute(.foregroundColor, value: color, range: fullRange)
                    
                    continuation.resume(returning: mutableAttributedString as NSAttributedString)
                } catch {
...
class AsyncTextView: UITextView {
    ...
    func setupUI() {
        ...
        // Observar cambios en el tamaño de texto
    ...
    @objc private func contentSizeCategoryChanged() {
        // Recargar el texto con el nuevo tamaño cuando cambie la categoría
        if let originalText = originalText {
            setAttributedTextAsync(
                text: originalText,
                font: currentFont,
    ...
    func setAttributedTextAsync(
        ...
        // Guardar los parámetros para poder recargar cuando cambie el tamaño de texto
        ...
        // Determinar la fuente a usar temporalmente mientras se carga el HTML
        let tempFont: UIFont
        if let textStyle = textStyle {
            if let baseFont = font {
                tempFont = UIFontMetrics(forTextStyle: textStyle).scaledFont(for: baseFont)
            } else {
                tempFont = UIFont.preferredFont(forTextStyle: textStyle)
            }
        } else if let font = font {
            if let descriptor = font.fontDescriptor.fontAttributes[.textStyle] as? UIFont.TextStyle {
                tempFont = UIFontMetrics(forTextStyle: descriptor).scaledFont(for: font)
            } else {
                tempFont = font
            }
        } else {
            tempFont = UIFont.preferredFont(forTextStyle: .body)
        }
        ...
        currentTask = Task {
            do {
                ...
                guard let text else {
                    throw String.AttributedStringError.nilString
                }
                let attributedString = try await text.toAttributedStringAsync(
                    font: font,
                    textStyle: textStyle,
                    color: color
                )
                ...
                // Asegurar que la fuente sea escalable después de establecer el attributedText
                self.font = tempFont
                self.adjustsFontForContentSizeCategory = true
            } catch {
                // El error ya está manejado mostrando el texto plano
            }
        }
    }
}

// Extension auxiliar para convertir UIColor a hex string
extension UIColor {
