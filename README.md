...
struct ContentModel: Hashable {
    ...
    init(logo: UIImage? = nil, tag: String? = nil, title: String, subtitle: String, description: String, isPremium: Bool = false) {
        self.logo = logo
        self.tag = tag
        self.title = title
        self.subtitle = subtitle
...
extension ContentModel {
    ...
    static func gridItem(index: Int, prefix: String, isPremium: Bool = false) -> ContentModel {
        if isPremium {
            return ContentModel(
                logo: UIImage(systemName: "photo.fill"),
                tag: "Premium",
                title: "\(prefix) - Celda Premium (\(index))",
...
class SectionDataManager {
    ...
    init(isValueEnabled: Bool = false, isVideoEnabled: Bool = false) {
        ...
        set {
            ...
            case .carousel(let header, _):
                if let newItems = newValue as? [ContentModel] {
                    ...
                    // Para carousel, podrías necesitar sincronizar todos los items
                }
            case .grid(let header, var items):
                if let newItem = newValue as? ContentModel, items.indices.contains(indexPath.item) {
                    ...
                    modelToSync = newItem // Marcar para sincronización
                }
            ...
            // Sincronizar ContentModel si fue actualizado
    ...
    subscript(headerFor sectionIndex: Int) -> SectionHeader? {
        get {
            ...
            // Actualizar dinámicamente el tabBar si es necesario
        ...
        set {
            ...
            // Actualizar el header dentro del content
    ...
    func shouldUsePremiumCell(at indexPath: IndexPath) -> Bool {
        ...
        case .grid(_, let items):
            ...
            return isValueEnabled && indexPath.item == 0 && item.isPremium
        default:
            return false
        }
    }
    ...
    /// Actualiza múltiples ContentModel de una vez
    ...
    func findContentModels(withId id: UUID) -> [(tab: TabItem, section: Int, item: ContentModel)] {
        var results: [(tab: TabItem, section: Int, item: ContentModel)] = []
            ...
            for (sectionIndex, section) in sections.enumerated() {
                ...
                case .carousel(_, let items):
                    if let item = items.first(where: { $0.id == id }) {
                        results.append((tab: tab, section: sectionIndex, item: item))
                    }
    ...
    func updateContentModelProperty(withId id: UUID, update: (inout ContentModel) -> Void) {
        // Buscar el primer modelo con ese id
        var foundModel: ContentModel?
        for (_, sections) in sectionsByTab {
            for section in sections {
                switch section.content {
                case .carousel(_, let items):
                    if let model = items.first(where: { $0.id == id }) {
                        foundModel = model
                        break
                    }
                case .grid(_, let items):
        ...
        // Aplicar la actualización y sincronizar
        if var model = foundModel {
            update(&model)
            updateContentModel(model)
        }
    }
    ...
    private func createSections(for tab: TabItem) -> [Section] {
        ...
        let itemCount = 15 + (Int(tab.title.last?.wholeNumberValue ?? 1) * 3)
        return [
    ...
    private func createGridSection(prefix: String, itemCount: Int) -> Section {
        let items = (0..<itemCount).map { i in
            ContentModel.gridItem(index: i, prefix: prefix, isPremium: self.isValueEnabled && i == 0)
        }
    ...
    /// Actualiza los items de una sección específica
    func updateItems(for sectionIndex: Int, with newItems: Any) {
        guard var section = self[section: sectionIndex] else { return }
    ...
    /// Obtiene los items de una sección
    func getItems(for sectionIndex: Int) -> Any? {
    ...
    /// Recarga los datos de una tab específica
...
final class PremiumCell: UICollectionViewCell {
...
final class ViewController: UIViewController {
    ...
    private func registerCellsAndHeaders() {
        ...
        collectionView.register(PremiumCell.self, forCellWithReuseIdentifier: PremiumCell.reuseIdentifier)
...
extension ViewController {
    ...
    private func createGridSection(for sectionIndex: Int, layoutEnvironment: NSCollectionLayoutEnvironment) -> NSCollectionLayoutSection {
        let hasPremiumCell = sectionDataManager.shouldUsePremiumCell(at: IndexPath(item: 0, section: sectionIndex))
        let section = hasPremiumCell ?
        createPremiumGridSection(for: sectionIndex, layoutEnvironment: layoutEnvironment) :
    ...
    private func createPremiumGridSection(for sectionIndex: Int, layoutEnvironment: NSCollectionLayoutEnvironment) -> NSCollectionLayoutSection {
        ...
        if isIPad {
            subitems = createIPadLayout(itemCount: itemCount, finalHeight: finalHeight)
        } else if isPortrait {
            subitems = createPortraitLayout(itemCount: itemCount, finalHeight: finalHeight)
        } else {
            subitems = createLandscapeLayout(itemCount: itemCount, finalHeight: finalHeight, layoutEnvironment: layoutEnvironment)
        }
        
        let containerGroupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1.0), heightDimension: .estimated(2000))
    ...
    private func createIPadLayout(itemCount: Int, finalHeight: CGFloat) -> [NSCollectionLayoutGroup] {
        var subitems: [NSCollectionLayoutGroup] = []
        if itemCount > 0 {
        ...
        let remainingItems = itemCount - 1
        if remainingItems > 0 {
            ...
            for _ in 0..<(remainingItems / 3) { subitems.append(tripletGroup) }
    ...
    private func createPortraitLayout(itemCount: Int, finalHeight: CGFloat) -> [NSCollectionLayoutGroup] {
        var subitems: [NSCollectionLayoutGroup] = []
        if itemCount > 0 {
            let fullWidthItemSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1.0), heightDimension: .absolute(finalHeight))
            let fullWidthItem = NSCollectionLayoutItem(layoutSize: fullWidthItemSize)
        ...
        if remainingItems > 0 {
            ...
            for _ in 0..<(remainingItems / 2) { subitems.append(pairGroup) }
    ...
    private func createLandscapeLayout(itemCount: Int, finalHeight: CGFloat, layoutEnvironment: NSCollectionLayoutEnvironment) -> [NSCollectionLayoutGroup] {
        var subitems: [NSCollectionLayoutGroup] = []
        let availableWidth = layoutEnvironment.container.effectiveContentSize.width - 32
        if itemCount > 0 {
            let standardColumnWidth = (availableWidth - (2 * cellSpacing)) / 3
            let premiumItemSize = NSCollectionLayoutSize(widthDimension: .absolute(standardColumnWidth * 2 + cellSpacing), heightDimension: .fractionalHeight(1.0))
            let premiumItem = NSCollectionLayoutItem(layoutSize: premiumItemSize)
            let regularItemSize = NSCollectionLayoutSize(widthDimension: .absolute(standardColumnWidth), heightDimension: .fractionalHeight(1.0))
            let regularItem = NSCollectionLayoutItem(layoutSize: regularItemSize)
            let firstRowGroupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1.0), heightDimension: .absolute(finalHeight))
            if itemCount > 1 {
                let firstRowGroup = NSCollectionLayoutGroup.horizontal(layoutSize: firstRowGroupSize, subitems: [premiumItem, regularItem])
                firstRowGroup.interItemSpacing = .fixed(cellSpacing)
                subitems.append(firstRowGroup)
            } else {
                let firstRowGroup = NSCollectionLayoutGroup.horizontal(layoutSize: firstRowGroupSize, subitems: [premiumItem])
                subitems.append(firstRowGroup)
            }
        }
        let remainingItems = itemCount - 2
        if remainingItems > 0 {
            let standardItemSize = NSCollectionLayoutSize(widthDimension: .absolute((availableWidth - (2 * cellSpacing)) / 3), heightDimension: .fractionalHeight(1.0))
            let standardItem = NSCollectionLayoutItem(layoutSize: standardItemSize)
            let tripletGroupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1.0), heightDimension: .absolute(finalHeight))
            for _ in 0..<(remainingItems / 3) {
                let tripletGroup = NSCollectionLayoutGroup.horizontal(layoutSize: tripletGroupSize, subitems: [standardItem, standardItem, standardItem])
                tripletGroup.interItemSpacing = .fixed(cellSpacing)
...
extension ViewController {
    ...
    private func calculateMaxGridCellHeight(for cellWidth: CGFloat, in sectionIndex: Int) -> CGFloat {
        ...
        let premiumSizingCell = PremiumCell(frame: .zero)
        ...
        for (index, item) in items.enumerated() {
            ...
            if sectionDataManager.shouldUsePremiumCell(at: indexPath) {
                premiumSizingCell.configure(with: item)
                sizingView = premiumSizingCell
            } else {
...
extension ViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case .grid(_, let items):
            ...
            if sectionDataManager.shouldUsePremiumCell(at: indexPath) {
                let cell = collectionView.dequeueReusableCell(withReuseIdentifier: PremiumCell.reuseIdentifier, for: indexPath) as! PremiumCell
                cell.configure(with: item)
                return cell
            } else {
                let cell = collectionView.dequeueReusableCell(withReuseIdentifier: TitleSubtitleCell.reuseIdentifier, for: indexPath) as! TitleSubtitleCell
